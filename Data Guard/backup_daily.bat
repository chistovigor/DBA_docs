echo off
rem **********************************
rem ***rm_backup_daily_compress 1.1***
rem **********************************
...
rem script name
Set _ScriptName=rm_backup_daily_compress_%_TargetName%
rem operation name
Set _OperName=Database Compressed Backup Level 1 
...
echo ### Making Database Backup Level 1 >> "%_ScriptName%.rcv"
...
echo backup as compressed backupset incremental level 1 database include current controlfile PLUS ARCHIVELOG format '%_BackupDir%\%_Today%\archlogs_db1_%%T_%%d_%%U.rm' SKIP INACCESSIBLE; >> "%_ScriptName%.rcv"
echo } >> "%_ScriptName%.rcv"
echo # >> "%_ScriptName%.rcv"
echo ### Listing and Clearing Obsolete >> "%_ScriptName%.rcv"
