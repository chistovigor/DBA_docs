...
if [[ `echo $db_status` == PRIMARY ]]; then
  ...
  $test_run ALTER DATABASE RECOVER MANAGED STANDBY DATABASE DISCONNECT FROM SESSION;
...
EOF
  ...
  echo -e "${Red}"
  echo !!!run the following command as `whoami` user on the server $remote_host
  echo sed "'$primary_string'" /etc/oratab
  echo -e "${Color_Off}"
   else
    echo -e "${Red}"
