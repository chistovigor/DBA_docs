...
RMAN_LOG_FILE=/export/home/oracle/scripts/rman/log/dmartdb_backup.log_$DATE
...
export ORACLE_SID
#$RMAN target $TARGET_CONNECT_STR catalog rman_cat/Dubai2020#@catlog msglog $RMAN_LOG_FILE append << EOF
$RMAN target $TARGET_CONNECT_STR catalog rman_way4db/Dubai2020#@catlog2 msglog $RMAN_LOG_FILE append <<EOF
...
SEND 'NB_ORA_CLIENT=unidmartb,NB_ORA_SID=way4dwhp,NB_ORA_POLICY=SOL_NI_DEIRA_UNIDMARTB_DB,NB_ORA_SERV=unibkpprd1,NB_ORA_SCHED=Default-Application-Backup';
BACKUP
    $BACKUP_TYPE
    SKIP INACCESSIBLE
...
#
ALLOCATE CHANNEL ch00 TYPE 'SBT_TAPE';
SEND 'NB_ORA_CLIENT=unidmartb,NB_ORA_SID=way4dwhb,NB_ORA_POLICY=SOL_NI_DEIRA_UNIDMARTB_DB,NB_ORA_SERV=unibkpprd1,NB_ORA_SCHED=Default-Application-Backup';
BACKUP
    # recommended format
...
delete FORCE NOPROMPT archivelog all backed up 1 times to sbt_tape;
EOF
"
...
STATUS=`cat $RMAN_LOG_FILE|egrep -i "error|ORA-|RMAN-"|wc -l`
if [ $STATUS == 0 ]
then
echo "Backup for $ORACLE_SID has completed Successfully"
#mailx -s "Backup for $ORACLE_SID has completed Successfully at $DATE" munafma@network.ae,milans@network.ae < tmp.log
touch /export/home/ora1/scripts/rman/tmp.log
mailx -s "Backup for $ORACLE_SID has completed Successfully at $DATE" munafma@network.ae,milans@network.ae,OracleDBA@network.ae </export/home/oracle/scripts/rman/tmp.log
else
echo "Error in Backup for $ORACLE_SID"
mailx -s "Backup for $ORACLE_SID has Error at $DATE" munafma@network.ae,milans@network.ae,OracleDBA@network.ae < $RMAN_LOG_FILE
fi
