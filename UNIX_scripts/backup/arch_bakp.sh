oracle@unidmartb:~/scripts/rman$ cat rman_archive_log_backup.sh
#!/bin/sh
...
RMAN_LOG_FILE=/export/home/oracle/scripts/rman/log/dmart_arch_backup.log_$DATE
...
SEND 'NB_ORA_CLIENT=unidmartb,NB_ORA_SID=way4dwhp,NB_ORA_POLICY=SOL_NI_DEIRA_UNIDMARTB_ARCH_HLY,NB_ORA_SERV=unibkpprd1,NB_ORA_SCHED=Default-Application-Backup';
BACKUP
   filesperset 1
   FORMAT 'al_%s_%p_%t'
...
touch /export/home/oracle/scripts/rman/tmp.log
mailx -s "Archive Backup for $ORACLE_SID has completed Successfully at $DATE" munafma@network.ae,milans@network.ae,OracleDBA@network.ae </export/home/oracle/scripts/rman/tmp.log
else
echo "Error in Backup for $ORACLE_SID"
mailx -s "Archive Backup for $ORACLE_SID has Error at $DATE" munafma@network.ae,milans@network.ae,OracleDBA@network.ae < $RMAN_LOG_FILE
fi
