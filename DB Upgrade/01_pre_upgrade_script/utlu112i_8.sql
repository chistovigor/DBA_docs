Rem
Rem $Header: rdbms/admin/utlu112i.sql ewittenb_bug-17288409/2 2013/09/10 12:54:31 ewittenb Exp $
Rem
Rem utlu112i.sql
Rem
...
Rem    cmlim       01/28/13 - for upload as utlu112i_8.sql on MOS to fix
Rem                           bug 18140608 - make sure the "Miscellaneous
Rem                           Warnings" secton is always displayed, even if
Rem                           no such conditions exist
...
-- *****************************************************************
  ...
  utlu_buildrev    CONSTANT VARCHAR2(3)  := '008';
...
BEGIN
   ...
   ELSE
      -- bug 18140608: The condition list here is not complete.  So
      --               let's just get rid of the IF check and make sure
      --               this Miscellaneous Warnings section is always displayed,
      --               regardless of whether such conditions exist or not.
      --               At worst case, there's just an empty Miscellaneous
      --               Warnings section.
      --
      ...
      -- THEN
            ...
            display_line('<Note: Section can be empty if no such warnings are found.>');
            display_banner;
      -- ELSE
      --    RETURN;
      -- END IF;
