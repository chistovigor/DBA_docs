...
DECLARE
  ...
  CURSOR desc_cur IS SELECT column_name FROM all_tab_columns WHERE 
...
BEGIN
  ...
  sql_text:=sql_text||' (SELECT rownum item,row_length FROM (SELECT ';
  ...
  LOOP
    ...
    LOOP
        IF J>I THEN
          sql_text:=sql_text||'*';
        END IF;
        sql_text:=sql_text||'NVL2('||desc_table(J).column_name||',0,1)';
    END LOOP;
    sql_text:=sql_text||',0,1,0)';
  END LOOP;
  sql_text:=sql_text||'+3 row_length FROM &OWNER'||'.&TABLE_NAME WHERE '||WhereCond||' ORDER BY row_length)) t,';
  sql_text:=sql_text||'(SELECT COUNT(*) row_num FROM &OWNER'||'.&TABLE_NAME WHERE '||WhereCond||') n ';
  ...
  DBMS_OUTPUT.PUT_LINE('TABLE: '||UPPER('&OWNER')||UPPER('.&TABLE_NAME'));
